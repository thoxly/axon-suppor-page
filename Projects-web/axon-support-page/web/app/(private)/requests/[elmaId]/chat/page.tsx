...
export default function ChatPage({
  ...
  const handleCloseRequest = async () => {
    ...
    if (!id || id === "undefined" || id === "null") {
      setCloseError("Некорректный идентификатор заявки");
      return;
    }
    ...
    try {
      const response = await fetch(`/api/requests/${id}/solve`, {
        method: "POST",
      });
  ...
  return (
    <div className="flex h-[calc(100vh-6rem)] flex-col rounded-xl border border-slate-200 bg-white">
      ...
      {canPost && (
        <div className="border-b border-slate-200 bg-white px-4 py-2 text-right">
          <button
            type="button"
      ...
      <div
        className="flex-1 overflow-y-auto px-4 py-3 text-xs bg-[color:var(--background)]"
        ref={bottomRef}
      >
        {loading && (
        ...
        {!loading && error && (
          <p className="text-center text-rose-500" role="alert">
            {error}
          </p>
        )}
        ...
        {closeError && (
          <p className="mb-2 text-center text-[11px] text-rose-500" role="alert">
            {closeError}
          </p>
        )}
      ...
      >
        <div className="flex items-end gap-2">
          <textarea
            className="min-h-[40px] flex-1 resize-none rounded-lg border border-slate-300 bg-white px-3 py-2 text-xs text-slate-900 outline-none placeholder:text-slate-500 focus:border-sky-500 focus:ring-1 focus:ring-sky-500 disabled:cursor-not-allowed disabled:bg-slate-100"
            placeholder={
              canPost
          ...
          <button
            ...
            className="inline-flex items-center rounded-lg bg-sky-500 px-3 py-2 text-[11px] font-medium text-white shadow-sm transition hover:bg-sky-400 disabled:cursor-not-allowed disabled:bg-slate-200 disabled:text-slate-400"
          >
            Отправить
          </button>
        </div>
